...
```
email: "userbaru@example.com"  (optional)
nama: "Nama Baru"            (atau gunakan key: name)
avatar: [image file]          (optional)
kelas: "X|XI|XII"            (optional, siswa only)
jurusan_id: 1                 (optional, siswa only)
```

Catatan:
- Untuk ganti password gunakan endpoint `PUT /profile/password`.
- Backend menerima `nama` maupun `name` untuk update nama (alias).
