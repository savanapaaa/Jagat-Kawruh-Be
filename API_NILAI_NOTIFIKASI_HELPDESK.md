...
### 3. Export Nilai to Excel (Guru/Admin only)
Download nilai dalam format Excel (`.xlsx`).

```
GET /nilai/export?type=all&kelas_id=1
```

**Query Parameters:**
- `type` (optional): `kuis`, `pbl`, `all` (default: all)
- `siswa_id` (optional): filter untuk satu siswa
- `kelas` (optional): filter nama kelas (contoh: `XI RPL 1`)
- `kelas_id` (optional): filter id kelas (integer)

**Response:**
- `200` File download: `application/vnd.openxmlformats-officedocument.spreadsheetml.sheet`

**Notes:**
- Sheet `Kuis` berisi nilai attempt kuis (submitted/expired)
- Sheet `PBL` berisi nilai submission PBL yang sudah dinilai
- Kolom export: `Judul`, `Nama Siswa`, `Kelas`, `Nilai`
